...
class myDataset(data.Dataset):
    ...
    def _set_files(self):
        self.image_dir = osp.join(self.root,'train')
        file_list = osp.join(
    ...
    def __getitem__(self, index):
        ...
        image_path_list = glob(osp.join(self.image_dir, image_id, '*'))
        image_path_list.sort()
        image_array = []
        for image_path in image_path_list:
            # print(image_path)
            if 'seg' in image_path: # label
        ...
        # image = zoom(image,(1,1,256/h,256/w))
